...
inline VectorXd ridge(const MatrixXd&X, const VectorXd&y, double lambda)
{
...
inline void kfold_cv_ridge(const MatrixXd& X_inp, const VectorXd& y, int k, double lb, double ub, double stepsize, int nochange)
{
...
inline void kfold_cv_ridge(const MatrixXd& X, const VectorXd& y, int k)
{
    kfold_cv_ridge(X,y,k,-4,10,0.01,7);
}

inline void kfold_cv_ridge(const MatrixXd& X, const VectorXd& y, int k, double lb, double ub, int n, int nochange)
{
    double stepsize=(ub-lb)/(n-1);
    kfold_cv_ridge(X,y,k,lb,ub,stepsize,nochange);
}

inline void kfold_cv_ridge(const MatrixXd& X, const VectorXd& y, int k, double lb, double stepsize, int n, int nochange)
{
