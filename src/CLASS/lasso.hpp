...
#include <random>
#include <omp.h>
#include "../lib/regression_metrics.hpp"

using namespace std;
using namespace Eigen;

/**
 * @brief This is a helper function used to divide the data into folds randomly for k-fold cross validation.
 * 
 * The function initializes a vector of indices, corresponding to the rows of the data. 
 * It shuffles these indices and then creates folds by grouping consecutive indices.
 * 
 * @param X Contains the data
 * @param y Contains the target
 * @param k This is the number of folds the function divides the data into
 * @param X_tra This is a vector which contains all the training folds of the data
 * @param X_test This is a vector which contains all the test folds of the data
 * @param Y_tra This is a vector which contains all the training folds of the target
 * @param Y_test This is a vector which contains all the test folds of the target
 */
inline void dividing_data_for_cv(const MatrixXd& X, const VectorXd& y, int k, vector<MatrixXd>& X_tra, vector<MatrixXd>& X_test, vector<VectorXd>& Y_tra, vector<VectorXd>& Y_test)
{
    int si=X.rows()/k;
...
inline void coord_descent(const MatrixXd&X, double lambda, VectorXd& b, int k,const VectorXd& r, double ss)
{
 ...
 * @return VectorXd 
 */
inline VectorXd lasso(const MatrixXd&X, const VectorXd&y, double lambda, int maxiter, double tol, int miniter)
{
...
inline VectorXd lasso(const MatrixXd&X, const VectorXd&y, double lambda)
{
    return lasso(X,y,lambda,200,0.0001,30);
}


inline VectorXd kfold_cv_lassomain(const MatrixXd& X_inp, const VectorXd& y, int k, double lb, double ub, double stepsize, int nochange, int maxiter, double tol, int miniter)
{
...
inline VectorXd kfold_cvlasso(const MatrixXd& X, const VectorXd& y, int k)
{
