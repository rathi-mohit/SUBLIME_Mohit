...
inline VectorXd LinearRegression(const MatrixXd& X,const VectorXd& y)
{
    
    return (X.transpose()*X).ldlt().solve(X.transpose()*y);
}

inline VectorXd betaOLS(MatrixXd &X, VectorXd &y, bool intercept = true) {
    if (intercept) {
        return (X.transpose() * X).ldlt().solve(X.transpose() * y);
    }
    else {
...
inline VectorXd betaLasso(MatrixXd &X, VectorXd &y, double lambda, double res = 1e-4, int max_iter = 1000, bool intercept = true) {
    int N = X.rows();
    int p = X.cols();
...
inline void display(string a, double para_min, int para_minpos,const vector<VectorXd>& M,const vector<vector<int>>& M_list)
{
...
inline void forwardsubsetselection(const MatrixXd& X,const VectorXd& y)
{
    int p=X.cols()-1;
    vector<int> v(p);
    VectorXd AIC(p);
