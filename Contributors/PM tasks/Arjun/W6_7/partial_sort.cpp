...
public:
    ...
    pair<MatrixXd, VectorXd> select()
    {
        ...
        for (int j = has_intercept ? 1 : 0; j < p; ++j) {
            ...
            // Copy coz we will sort separately for top and bottom
            vector<pair<double, int>> column_values_copy = column_values;
            
            // Bottom r
            partial_sort(column_values.begin(), 
                        column_values.begin() + r, 
                        column_values.end(),
            ...
            // Top r
            partial_sort(column_values_copy.begin(), 
                        column_values_copy.begin() + r, 
                        column_values_copy.end(),
                        [](const pair<double, int>& a, const pair<double, int>& b) {
                            return a.first > b.first;
                        });
            ...
            #pragma omp critical
            {
                ...
                for (int i = 0; i < r; ++i) {
                    selected[column_values_copy[i].second] = true;
                }
            }
        }
